...
func Get_Recipe_Details(c *fiber.Ctx, db *sql.DB) error {
	...
	err = get_recipe_details(db, reqData.Recipe_Id, &response.Recipe)
	if err != nil {
		log.Println("get_recipe_details | Error on get_recipe_details: ", err.Error())
	...
	if err != nil {
		log.Println("get_recipe_images | Error on get_recipe_images: ", err.Error())
		return utilities.Send_Error(c, "An error occured in fetching recipe", fiber.StatusInternalServerError)
	}
...
func get_recipe_details(db *sql.DB, recipe_id uint, recipe *models.Recipe) error {
	...
	row := db.QueryRow(`SELECT 
			...
			prep_time,
			description
		FROM recipe WHERE id = $1`, recipe_id)
	err := row.Scan(
		&recipe.ID,
		&recipe.Name,
		&recipe.Name_Ph,
